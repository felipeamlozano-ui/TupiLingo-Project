...
def check_and_grant_xp_achievements(user: UserProfile) -> list[Achievement]:
    ...
    for ach in eligible:
        ...
        unlocked.append(ach)
        logger.info(
            "Usuário %s (%s) desbloqueou achievement: %s",
            user.id,
            user.email,
            ach.codigo,
        )
...
def check_and_grant_lesson_achievements(
    user: UserProfile, licao: Licao, accuracy: float
) -> list[Achievement]:
    """
    ...
    if (
        cap_codigo in [a["codigo"] for a in DEFAULT_ACHIEVEMENTS]
        and cap_codigo not in existing_codigos
    ):
        ...
        if total_licoes > 0 and licoes_concluidas >= total_licoes:
            ...
            if ach:
                ...
                unlocked.append(ach)
                logger.info(
                    "Usuário %s completou Capítulo %s! Conquista: %s",
                    user.id,
                    capitulo.numero,
                    cap_codigo,
                )
...
def get_all_achievements_with_status(
    user: UserProfile,
